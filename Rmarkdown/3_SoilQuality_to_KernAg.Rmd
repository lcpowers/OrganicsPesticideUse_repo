...
Commented out because can just read in 'soil_ras.tif' at the end of the next code chunk
```{r prepare_StorieIndex_shapefile}
# ## Read in Storie Index Soil Info
# soil_raw = read_sf("../R_input/spatial/KernSoilDataBasin/KernSoilData_QGISreproj.shp")
# 
# ## Put into projection that makes it overlap with Kern Ag and ultimately all over spatial data
# soil_raw = st_transform(soil_raw,"+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=23 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs") %>% 
#   dplyr::select(StorInd) ## Filter to keep only the column with Storie Index information and geometry (Geometry stays by default unless the dataframe is switched to a basic DF and geometries are removed)
# 
