...
topCrops_df = organics_sf %>% 
  ...
  summarise(total_fields = n(),
            ...
            total_convFields = n() - sum(cdfa_rg)) %>% 
...
for(i in years){
     ...
     tmp = organics_sf %>%  # Call dataframe that corresponds to the correct buffer width and year
       ...
       filter(cdfa_rg == "1" & comm_dt %in% comparable_crops) %>% # Filter for CDFA Organic fields
       group_by(comm_dt) %>% # Group by the commodity
       summarise(total_fields = n(), # Count total number of fields for each commodity
                 total_hectares = round(sum(hectars/N_crops),1), # Count hectares for each commodity
                 average_field_size = round(mean(hectars/N_crops),1), # Find average field size for each commodity
                 average_SoilQ = mean(sl_qlty, na.rm = T), # Find average soil quality for each commodity
                 total_pest_app = sum(KgPstAI), # Total pesticide applied for this crop
                 kg_per_hectare = sum(KgPstAI)/sum(hectars/N_crops)) %>% # Pesticide per hectare
...
for(i in years){
     ...
     tmp = organics_sf %>% 
       as.data.frame() %>% 
       filter(year == i) %>% 
       filter(cdfa_rg == "0" & comm_dt %in% comparable_crops) %>% # Filter for nonCDFA (conventional) fields and the five most common oganic crops 
       group_by(comm_dt) %>% 
       summarise(total_fields = n(),
                 total_hectares = round(sum(hectars/N_crops),1),
                 average_field_size = round(mean(hectars/N_crops),1),
                 average_SoilQ = mean(sl_qlty, na.rm = T),
                 total_pest_app = sum(KgPstAI),
                 kg_per_hectare = sum(KgPstAI)/sum(hectars/N_crops)) %>% 
