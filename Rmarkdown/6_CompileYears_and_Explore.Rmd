...
#### 2. Read in data for each year and compile for output CSV
```{r data}
...
all_years_df <- data.frame(matrix(ncol=40,nrow=0))
...
colnames(all_years_df) = c("permitsite","s_status","permittee","comm_original","agroclass","genus","family","comm_edit",
                        "acres","soil_quality","apn_org","trs_org","cdfa_org","KernComm_org","all_org","year",
                        "KgPestAI","KgPestPr","KgAerialAI","GrndAI","OtherAI","KgAiWater","KgAWildlife","KgAMammal",
                        "KgABirds","KgAiFish", "KgAiBees", "KgAEndgSp", "KgAAiqSp", "KgAGrdWat","KgADrift","KgAReptAmph","acrestreated" , 
                        "KgAInsectOnly","KgAHerbOnly","KgAFungOnly","AiInsFng","N_crops","hectares","KgPerHE")

write_csv(all_years_df, "../R_output/CSV/compiled_organics1317.csv")
```

#### Do the same as above then compile into single shapefile
```{r compile_for_shp}
...
colnames(all_years_shp) = c("permitsite","s_status","permittee","comm_original","agroclass","genus","family","comm_edit",
                        "acres","soil_quality","apn_org","trs_org","cdfa_org","KernComm_org","all_org","year",
                        "KgPstAI","KgPstPr","PgArlAI","GrndAI","OtherAI","KgAiWtr","KgAWldl","KgAMmml",
                        "KgABrds","KgAiFsh", "KgAiBes", "KgAEndS", "KgAAqSp", "KgAGrdW","KgADrft","KgARptA","acrstrt" , 
                        "KgAInsO","KgAHrbO","KgAFngO","AInsFng","N_crops","hectares","KgPerHE","geometry")
...
ggplot(mean_hectares,aes(x = cdfa_org,y = mean_hectares,fill=as.factor(cdfa_org)))+
  geom_col()+
...
mean_hectares = all_years_df %>% 
  group_by(year,cdfa_org) %>% 
  summarise(mean_soilQ = round(mean(soil_quality),2))

ggplot(mean_hectares,aes(x = cdfa_org,y = mean_soilQ,fill=as.factor(cdfa_org)))+
  geom_col()+
...
explore_pst_use = all_years_df %>% 
  ...
  summarise(avg_kg_pest_ai = round(mean(KgPstAI),2), # should adjust this for number of fields
            avg_kg_ai_perHec = round(mean(KgPerHE),2))
