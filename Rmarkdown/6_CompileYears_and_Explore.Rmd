...
mpdata = read_dta("../R_input/dta/organicAnalysisPURcopy.dta") %>% as.data.frame()

#Crosswalk for yield gaps excel sheet
yieldclass<-mpdata %>% dplyr::select(agroclass) %>% distinct()
  
map_data<-mpdata %>% filter(year==2019) %>% dplyr::select(permitsite, year, hectares, soil_quality, comm_edit, comm_code2, KgPestAI)%>% dplyr::mutate(KgPestAIha=KgPestAI/hectares) %>% dplyr::select(-KgPestAI)
