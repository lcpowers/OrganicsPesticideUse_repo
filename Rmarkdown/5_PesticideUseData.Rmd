...
pest_data_raw = read_stata("../R_input/CSV/KernOrgCollapse1319.dta") %>% 
  select(-permitsite)
pest_data_raw$permitsite = paste0(as.character(pest_data_raw$permitno),as.character(pest_data_raw$location))
...
for(i in years){
  ...
  anti_join = anti_join(tmp_pst,tmp_ag, by = c("permitsite" = "permtst"))
  
  join_pst$year = i
  
  #Replace NA in outcome variables with 0
  col_join_pst<-colnames(join_pst)[21:39] #Changed from 41 to 39 as only 39 columns
