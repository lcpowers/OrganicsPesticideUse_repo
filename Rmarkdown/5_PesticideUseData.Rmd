---
title: "Join PesticideUseData to Finazlied Organic Parcels Shapefile for all years"
output:
...
for(i in years){ # For each year...
  ...
  tmp <- readOGR(paste0("../R_output/spatial/CDFA_KernAg_join/",i,"/buffer",j,"/all_organics_",i,".shp")) %>% 
  st_as_sf()
...
for(i in years){
  ...
  join_pst = left_join(tmp_ag,tmp_pst, by = c("permtst" = "permitsite"))
  
  join_pst$year = i
  
  join_pst[17:37][is.na(join_pst[17:37])] <- 0
...
for(i in years){
  ...
  # Turn the 2017 kernAg_CDFA_pest shapefile into a simple features (spatial) data.frame
      ...
      tmp_orgs_df_final = merge(tmp_orgs_df, M_status_df_final, by = 'permtst')
      ...
      tmp_orgs_df_final$acres = as.numeric(as.character(tmp_orgs_df_final$acres))
      
      # create a 'hectares' column using the 'acres' column
      tmp_orgs_df_final$hectares = tmp_orgs_df_final$acres*0.405/tmp_orgs_df_final$N_crops_prmtst
      
      # Create a kilograms of active ingredients per hectare column
      tmp_orgs_df_final$kgperhec = tmp_orgs_df_final$KgPestAI/tmp_orgs_df_final$hectares
      
        # Write the shapefile in to the output directory
        writeOGR(as(tmp_orgs_df_final,"Spatial"),
