...
for(i in years){
  ...
  # Check to make sure all permits are 7 characters long -- need to double check this for all years
  ...
  # Merge the pesticide use data.frame to the Agriculture shapefile on 'permitsite' and 'commodity'
  merged_ag_pst = merge(tmp_ag,tmp_pst, by = c("permitsite","commodity"), suffix = c(".ag", ".pur"))
  
  # Vector of joined permitsites
  joined_permitsites = merged_ag_pst$permitsite
  ...
  ###########################
  leftovers_join = stringdist_inner_join(x = tmp_ag_leftovers,
                                         y = tmp_pst_leftovers,
                                         by = ("permitsite"), 
                                         max_dist = 1, 
                                         method = "osa",
                                         #mode = "inner", 
                                         distance_col = "dist") %>% 
    filter(permit.x == permit.y & commodity.x == commodity.y)
