...
### A) Read in Kern County Parcels that matched to CDFA records based on the APN
```{r cdfa_apn_match}
...
### B) Bring in Kern Sections shapefile
```{r PLSS_shapefile}
...
### C) Read in CDFA PLSS csv, join to PLSS shapefile and write output shapefile
```{r CDFA_trs}
...
for(i in yrs){
  ...
  cdfa_sections_df = as.data.frame(cdfa_sections_unique)
  
  assign(paste0("cdfa_trs_",i),cdfa_sections_df)
