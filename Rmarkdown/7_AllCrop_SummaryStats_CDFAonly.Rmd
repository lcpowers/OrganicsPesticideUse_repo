...
top_crops_df = organics_shp %>% 
  ...
  summarise(total_fields = n(),
            ...
            total_convFields = n() - sum(cdfa_rg)) %>% # Sum the CDFA column. This tells us how many organic fields of each crop type there are
  arrange(-total_orgFields) %>% # Sort the table in descending order of number of fields per crop type
  filter(total_orgFields >= 0 & total_convFields >= 0) # Get rid of crops with fewer than ten organic fields
