...
#### 3. Find organic crop types that have at least 5 fields/sites and 5 conventional fields of the same crop
```{r top_organic_crops}

yr = 2017

top_crops_df = organics_sf %>% 
  as.data.frame() %>% # Convert to data.frame
  filter(year == yr) %>% 
  group_by(comm_dt) %>% # Group by COMM column
  summarise(total_fields = n(),
            total_orgFields = sum(cdf_rgn),
            total_convFields = n() - sum(cdf_rgn)) %>% # Sum the CDFA column. This tells us how much organic fields of each crop type there are
...
comparison_crops = as.character(top_crops_df$comm_dt) # Create a new vector with names of crops with at least five fields
