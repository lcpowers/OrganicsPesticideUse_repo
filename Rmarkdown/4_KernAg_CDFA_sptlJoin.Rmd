...
for(i in years){
  ...
  ## Read in Kern Ag data with soil quality in attribute table, produced in RMD 3
...
for(i in years){
  ...
  tmp$comm_org <- ifelse(str_detect(tmp$comm,"ORG"),1,0)
  tmp$permittee_org <- ifelse(str_detect(tmp$permittee,"ORG"),1,0)
  print(sum(tmp$comm_org, na.rm = T))
