...
for(i in years){
  ...
  # Read in data produced above. This is the cdfa data joined to kern county parcels on APN, then spatially joined to kern ag data in the above function
   tmp <- read_sf(paste0("../R_output/spatial/CDFA_KernAg_join/",i,"/buffer50/KernAg_CDFA_all_",i,"_B50.shp"))
  
  assign(paste0("cdfa_kern_apn_",i),tmp)
  rm(tmp)
  
  tmp <- read_sf(paste0("../R_output/spatial/KernAg_withSoil/",i,"/KernAg_withSoil_",i,".shp")) %>%
    dplyr::select(PERMIT,PMT_SITE,S_STATUS,P_STATUS,COMM,PERMITTEE,AG_TRS)
  ...
  # Join kern_ag data to CDFA sections
  cdfasection_ag_join <- st_join(eval(as.name(paste0("kern_",i))),
                      eval(as.name(paste0("cdfa_sections_",i))),
                      largest=T)
  
  cdfasection_ag_join$trs_orgPermittee_match <- ifelse((str_detect(cdfasection_ag_join$PERMITTEE,"ORGANIC") & cdfasection_ag_join$trs_match == 1),1,0)
...
for(i in years){
  ...
  tmp$permittee_org <- ifelse(str_detect(tmp$permittee,"ORGANIC"),1,0)
  print(sum(tmp$comm_org))
  print(sum(tmp$permittee_org))
