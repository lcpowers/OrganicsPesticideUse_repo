...
kernfields_2019<-read_sf("../R_input/spatial/kern_AG_shp/kern2019/kern2019.shp") %>% dplyr::select(PMT_SITE, P_STATUS, S_STATUS) %>% dplyr::rename(permitsite=PMT_SITE)

kernfields_2019_spatial<-read_sf("../R_input/spatial/kern_AG_shp/kern2019/kern2019.shp") %>%dplyr::select(PERMIT) %>% distinct() %>% 
  dplyr::mutate(SpatialID=1:nrow(.)) 

kf_2019<-st_join(kernfields_2019_spatial, kernfields_2019)
