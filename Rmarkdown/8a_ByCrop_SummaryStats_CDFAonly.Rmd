...
years = 2017 # 2013:2017
#### This forloop reads in KernAg_CDFA_join shapefile, the shapefile that joined Kern Ag with CDFA parcels, for specific years and buffer widths ####
for(i in years){
    
    tmp = readOGR(paste0("../R_output/spatial/KernAg_CDFA_pest/",i,"/FINAL","/KernAg_CDFA_Pest_FINAL.shp")) %>% # Read in
      st_as_sf() # Convert to spatial dataframe
    
    # Create output name based on year and buffer width
...
```{r}
topCrops_df = data_2017 %>% 
  as.data.frame() %>% 
  dplyr::select(-geometry) %>% 
  group_by(comm_dt) %>% 
  summarise(total_orgFields = sum(organic)) %>% 
  arrange(-total_orgFields) %>% 
  filter(total_orgFields >= 15)

comparable_crops = topCrops_df$comm_dt
```
...
data_2017$acres = as.numeric(data_2017$acres)

for(i in years){
   for(j in buf_width){
    
     tmp = eval(as.name(paste0("data_",i))) %>%  # Call dataframe that corresponds to the correct buffer width and year
       as.data.frame() %>% # Remove the spatial characteristic of the dataframe
       dplyr::select(-geometry) %>% # Remove the geometry column
       filter(organic == "1" & comm_dt %in% comparable_crops) %>% # Filter for CDFA Organic fields
       group_by(comm_dt) %>% # Group by the commodity
       summarise(total_fields = n(), # Count total number of fields for each commodity
                 total_hectares = round(sum(acres*0.405/N_crps_),1), # Count hectares for each commodity
                 average_field_size = round(mean(acres*0.405/N_crps_),1), # Find average field size for each commodity
                 ...
                 kg_per_hectare = sum(KgPstAI)/sum(acres*0.405/N_crps_)) %>% # Pesticide per hectare
       arrange(comm_dt) # Arrange dataframe with total field column in descending order

     
     assign(paste0("ORG_",i,"_B",j,"_SumStats"),tmp)
     remove(tmp)
   }
}
...
for(i in years){
   for(j in buf_width){
     ...
     tmp = eval(as.name(paste0("data_",i))) %>% 
       as.data.frame() %>% 
       dplyr::select(-geometry) %>% 
       filter(organic == "0" & comm_dt %in% comparable_crops) %>% # Filter for nonCDFA (conventional) fields and the five most common oganic crops 
       group_by(comm_dt) %>% 
       summarise(total_fields = n(),
                 total_hectares = round(sum(acres*0.405/N_crps_),1),
                 average_field_size = round(mean(acres*0.405/N_crps_),1),
                 average_SoilQ = mean(storie, na.rm = T),
                 total_pest_app = sum(KgPstAI),
                 kg_per_hectare = sum(KgPstAI)/sum(acres*0.405/N_crps_)) %>% 
       arrange(comm_dt) # Arrange order of the 'crops' character vector
     
     assign(paste0("CONV_",i,"_B",j,"_SumStats"),tmp)
...
for(i in years){
   for(j in buf_width){
     ...
     assign(paste0("ALL_",i,"_B",j,"_SumStats"),tmp)
...
combined = cbind(ALL_2017_B50_SumStats,CONV_2017_B50_SumStats,ORG_2017_B50_SumStats) # Combined three outputs
...
write_csv(combined,"../R_output/CSV/SummaryStatistics/By_Crop_SummaryStatistics_CDFAonly.CSV") 
