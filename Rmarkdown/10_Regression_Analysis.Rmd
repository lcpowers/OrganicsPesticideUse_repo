...
data = readOGR("../R_output/spatial/final_organics_data/cdfa_with_KernAg_organics.shp") %>% 
  st_as_sf()
...
matching = data %>% 
  ...
  filter(organic_fields > 5 & conv_fields > 5) %>% 
...
propensity_logit = glm(organic ~ croptype + soil_q + hectars, 
                       family = binomial, 
...
match = matchit(organic ~ croptype + soil_q + hectars, 
                method = "nearest", 
                data = comm_regression_df)
...
```{r check_matches}
cov <- c('hectars', 'soil_q') # Covariates to check matches with
