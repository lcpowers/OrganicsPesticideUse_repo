...
Useful tutorial. There are two others in the repository folder now as well
https://sejdemyr.github.io/r-tutorials/statistics/tutorial8.html

#### Right now this is goes through the regression using 'agroclass' as the crop variable. I have also tried out 'comm_edit' and 'comm_group', which is where some of those notes are coming from. 
...
library(optmatch) #Additional package for optimal matching
...
# Get data and create dfs to be used in regression analysis. 
...
ag_summary = data %>% 
  ...
  filter(organic_fields > 9 & conv_fields > 9) %>% #Why 9?
  arrange(-organic_fields)

common_crops = as.factor(as.character(ag_summary$comm_edit)) #Crops with >0 fields conventional and organic for 2017
agro_classes = as.factor(as.character(data$agroclass[data$cdfa_org==1])) #Ag classes for organic 

comm_regression_df = data %>% 
  # filter(year == 2017) %>% 
  filter(soil_quality > 0 & comm_edit%in%common_crops) %>% #Soil greater >0 and for commodities defined by common_crops
  dplyr::select(permitsite,
         ...
         KgPstAI,
         KgPerHE,
         hectares) #Permit site based

agroclass_regression_df = data %>% 
  # filter(year == 2017) %>% 
  filter(soil_quality >0 & agroclass%in%agro_classes) %>% #Soil greater >0 and for ag classes found for organic fields
...
# Simple Linear Regression -- Exploring how R is recognizing/processing covariates
```{r SLR}
...
# Propensity score estimation for dataset using agroclass as crop variable
```{r estimate_scores}
### Currently with agro class ###

propensity_logit = glm(cdfa_org ~ agroclass + soil_quality + hectares + year, 
                       family = binomial, 
...
# Sofie: Whole section below does not work for me
# Claire: For some reason updated df names didn't save. Should work now?

agroclass_regression_df$pscores = predict.glm(propensity_logit, 
                                         type="response",
                                         newdata = agroclass_regression_df) 

#Can you use this equation on this dataset? Equation used other dataset

pscore_reg = lm(KgPerHE ~ agroclass + cdfa_org + hectares + soil_quality + pscores, data = agroclass_regression_df)
summary(pscore_reg)
...
matches_reg = lm(KgPerHE ~ agroclass + cdfa_org + hectares + soil_quality , data = matches_filter) 
summary(matches_reg)
