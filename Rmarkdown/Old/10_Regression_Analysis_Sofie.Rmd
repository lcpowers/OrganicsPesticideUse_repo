...
## Subset Data
```{r data_model, message=FALSE, warning=FALSE}

#Select crops that have at least 1 organic and conventional fields per year
  cropfields <- data %>% 
    group_by(comm_edit, year) %>% # Group by COMM column
    summarise(total_fields = n(),
              total_orgFields = sum(cdfa_org),
              total_convFields = n() - sum(cdfa_org)) %>% # Sum the CDFA column. This tells us how much organic fields of each crop type there are
    filter(total_orgFields >= 1 & total_convFields >= 1) %>%  # Get rid of crops with fewer than five organic/conventional fields
    ungroup()
  yearfields<-cropfields %>% 
    group_by(comm_edit) %>% 
    summarise(count=n()) %>% 
    filter(count==5) %>% 
    dplyr::select(comm_edit) %>% 
    ungroup()

#Subset datasets for crops/fields with at least 1 organic and 1 conventional field per year per crop
  data_model<-right_join(data, yearfields, by="comm_edit") %>% 
...
m_ps<-glm(cdfa_org~fieldsize + soilquality + croptype,
          data=data_model, family=binomial())
summary(m_ps)
#Propensity score model with year as factor
m_ps_yr<-glm(cdfa_org~fieldsize + soilquality + croptype +
               factor(year),
          data=data_model, family=binomial())
summary(m_ps_yr)
