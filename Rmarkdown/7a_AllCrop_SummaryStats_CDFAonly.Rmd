...
- This document and output are the same as in "7b_AllCrops_SummaryStats", except that the input for this file *only includes* organic fields identified by the CDFA APN *NOT* Kern Ag Shapefile attribute table ("ORGANIC" in the comm name)
...
#### 2. Read in full shapeile and convert to data.frame
```{r read_in_shapefiles}

crops_2017 = readOGR(paste0("../R_output/spatial/KernAg_CDFA_pest/2017/FINAL/KernAg_CDFA_Pest_FINAL.shp")) %>% 
  st_as_sf()
...
for(i in years){
     ...
     tmp = eval(as.name(paste0("crops_",i))) %>% 
       as.data.frame() %>% 
